...
- Migrate saved `feedback.github_repo` values of `Pythoughts-labs/pythinker-code` and `Pythoughts-labs/pythinker-cli` (earlier bundled defaults) to `PyModel/pythinker-cli` on config load, and point the built-in help skill's documentation URLs at the canonical `pythinker-cli` GitHub Pages path.
...
- Update all repository URLs, repo-slug constants, Homebrew tap and Scoop bucket owners, and the documentation site URL (`pymodel.github.io/pythinker-cli`) to the PyModel GitHub org, and name PyModel as the developer in the system prompt, `pythinker info`, and package metadata. The docs-pages deploy gate now matches `PyModel/pythinker-cli`, so documentation deploys resume.
